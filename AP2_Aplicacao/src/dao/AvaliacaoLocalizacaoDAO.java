...
import util.Restaurante; // Importe a classe Restaurante
import util.Cliente;     // Importe a classe Cliente
import bd.ConnectionFactory; // Importe sua ConnectionFactory
...
public class AvaliacaoLocalizacaoDAO implements BaseDAO {
    ...
    public void salvar(Object obj) {
        if (obj instanceof AvaliacaoLocalizacao) {
            ...
            // SQL para inserir uma nova avaliação de localização
            ...
            try (Connection conn = ConnectionFactory.getConnection();
                ...
                if (affectedRows > 0) {
                    try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
                        if (generatedKeys.next()) {
                            ...
                            System.out.println("Avaliação de Localização salva com sucesso no DB (ID: " + avaliacao.getIdAvaliacao() + ")");
                        } else {
                            System.err.println("Falha ao obter o ID gerado para a Avaliação de Localização.");
                        }
                    }
                } else {
                ...
                System.err.println("Erro ao salvar Avaliação de Localização no banco de dados: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    System.out.println("Avaliação de Localização encontrada: ID " + avaliacao.getIdAvaliacao());
                }
            }
        } catch (SQLException e) {
            System.err.println("Erro ao buscar Avaliação de Localização por ID: " + e.getMessage());
            e.printStackTrace();
        }
        return avaliacao;
    }
    ...
    public ArrayList<Object> listarTodosEagerLoading() {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            System.err.println("Erro ao listar Avaliações de Localização: " + e.getMessage());
            e.printStackTrace();
        }
        return avaliacoes;
    }
    ...
    public void atualizar(Object obj) {
        if (obj instanceof AvaliacaoLocalizacao) {
            ...
            try (Connection conn = ConnectionFactory.getConnection();
                ...
                int affectedRows = stmt.executeUpdate();
                if (affectedRows > 0) {
                    System.out.println("Avaliação de Localização com ID " + avaliacao.getIdAvaliacao() + " atualizada com sucesso.");
                } else {
                    System.out.println("Avaliação de Localização com ID " + avaliacao.getIdAvaliacao() + " não encontrada para atualização.");
                }
            } catch (SQLException e) {
                System.err.println("Erro ao atualizar Avaliação de Localização: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
    ...
    public void excluir(int id) {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            System.err.println("Erro ao excluir Avaliação de Localização: " + e.getMessage());
            e.printStackTrace();
        }
    }
}